...
(defun change-split (cngx) ; -> list of changes.
  ...
  (let (ret-lst m01 m10)
    ...
    (loop for bitx in m01 do
      (push (change-new :m01 bitx :m10 (mask-new-low (change-m01 cngx))) ret-lst)
    )

    (setf m10 (mask-split (change-m10 cngx)))
    (loop for bitx in m10 do
      (push (change-new :m10 bitx :m01 (mask-new-low (change-m01 cngx))) ret-lst)
    )
    ret-lst
  )
)
...
(defun change-and (cng1 cng2) ; -> change
  ...
  (change-new :m01 (mask-and (change-m01 cng1) (change-m01 cng2))
              :m10 (mask-and (change-m10 cng1) (change-m10 cng2)))
)
...
(defun change-not (cngx) ; -> change
  ...
  (change-new :m01 (mask-not (change-m01 cngx))
              :m10 (mask-not (change-m10 cngx)))
)
...
(defun change-or (cng1 cng2) ; -> change
  ...
  (change-new :m01 (mask-or (change-m01 cng1) (change-m01 cng2))
              :m10 (mask-or (change-m10 cng1) (change-m10 cng2)))
)
...
(defun change-remove-x-x-not (cngx) ; -> change
  ...
  (let ((mask-x-x-not (mask-and (change-m01 cngx) (change-m10 cngx))))

    (change-new :m01 (mask-and-not (change-m01 cngx) mask-x-x-not)
                :m10 (mask-and-not (change-m10 cngx) mask-x-x-not))
  )
)
