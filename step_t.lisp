...
(defun step-tests ()
  ...
  (let (stp1)
    ...
    (setf stp1 (step-new 0 (rule-from-str "[00/XX/01/Xx]")))
    (assert (eq (type-of stp1) 'STEP))

    ;(format t "~&stp1: ~A" (step-str stp1))

    (assert (= 0 (step-act-id stp1)))
    (assert (rule-eq (step-rule stp1) (rule-from-str "[00/XX/01/Xx]")))
  ...
  (let (stp1 stp2)
    (setf stp1 (step-new 0 (rule-from-str "[00/XX/01/Xx]")))
    (setf stp2 (step-new 0 (rule-from-str "[00/XX/01/Xx]")))
  ...
  (let (stp1 reg1 stp2)
    (setf stp1 (step-new 2 (rule-from-str "[00/01/11/10_X1/X1/X1_X0/X0/X0_Xx/Xx/Xx_XX/XX/XX]")))
    ...
    (assert (rule-eq (step-rule stp2) (rule-from-str "[00/01/11/10_01/11/X1_00/10/X0_01/10/Xx_00/11/XX]")))
  ...
  (let (stp1 reg1 stp2)
    (setf stp1 (step-new 2 (rule-from-str "[00/01/11/10_X1_X0_Xx/Xx/Xx_XX/XX/XX]")))
    ...
    ;(format t "~&stp2 ~A" (step-str stp2))
    (assert (= 2 (step-act-id stp2)))
    (assert (rule-eq (step-rule stp2) (rule-from-str "[00_01/11/10/X1_X0_10/01/Xx_00/11/XX]")))
