...
(defun value-tests ()
  ...
  (let (valx errx)
    ...
    (setf errx (value-from-na "1"))
    (assert (and (err-p errx) (string= (err-str errx) "String does not begin with the v character")))

    ; Test string for invalid binary digit.
    (setf errx (value-from-na "v012"))
    (assert (and (err-p errx) (string= (err-str errx) "Invalid binary digit 2")))
