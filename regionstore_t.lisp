...
(defun regionstore-tests ()
  ...
  (let (store1 store2 store3)
    ...
    (assert (regionstore-contains store2 (region-from 'rXXX0)))
    (assert (regionstore-contains store2 (region-from 'rXX0X)))
    (assert (regionstore-contains store2 (region-from 'rX0XX)))
    ...
    (assert (regionstore-contains store3 (region-from 'rX01X)))
    (assert (regionstore-contains store3 (region-from 'r10XX)))
    (assert (regionstore-contains store3 (region-from 'rX10X)))
    (assert (regionstore-contains store3 (region-from 'r1X0X)))
    (assert (regionstore-contains store3 (region-from 'rXX10)))
    (assert (regionstore-contains store3 (region-from 'rX1X0)))
    (assert (regionstore-contains store3 (region-from 'r1XX0)))
  ...
  (let (store1 store2 store3)
    ...
    (assert (regionstore-contains store3 (region-from 'rX000)))
    (assert (regionstore-contains store3 (region-from 'rX001)))
    (assert (regionstore-contains store3 (region-from 'rX011)))
  ...
  (let (regs1 ints)
    ...
    (assert (regionstore-contains ints (region-from 'r0X11)))
    (assert (regionstore-contains ints (region-from 'r0X01)))
  ...
  (let (regst1 fragments)
    ...
    (assert (regionstore-contains fragments (region-from 'r0101)))
    (assert (regionstore-contains fragments (region-from 'r0110)))
    (assert (regionstore-contains fragments (region-from 'r0101)))
    ...
    (assert (regionstore-contains fragments (region-from 'r0101)))
    (assert (regionstore-contains fragments (region-from 'r0110)))
    (assert (regionstore-contains fragments (region-from 'r0011)))
    (assert (regionstore-contains fragments (region-from 'r0111)))
    ...
    (assert (regionstore-contains fragments (region-from 'r0101)))
    (assert (regionstore-contains fragments (region-from 'r0110)))
    (assert (regionstore-contains fragments (region-from 'rX011)))
    (assert (regionstore-contains fragments (region-from 'r1X11)))
    (assert (regionstore-contains fragments (region-from 'r0111)))
    ...
    ;(format t "~&fragments ~A" fragments)
    (assert (= (regionstore-length fragments) 3))
    (assert (regionstore-contains fragments (region-from 'rX100)))
    (assert (regionstore-contains fragments (region-from 'rX111)))
    (assert (regionstore-contains fragments (region-from 'rX101)))
    ...
    (assert (regionstore-contains fragments (region-from 'rXXX0)))
    (assert (regionstore-contains fragments (region-from 'rX0XX)))
    (assert (regionstore-contains fragments (region-from 'r1111)))
    (assert (regionstore-contains fragments (region-from 'r0111)))
    (assert (regionstore-contains fragments (region-from 'r1101)))
    (assert (regionstore-contains fragments (region-from 'r0101)))
