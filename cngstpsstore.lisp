...
(defstruct cngstpsstore
  cngstps  ; A list of zero, or more, non-duplicate, same number bits, change steps.
)
...
(defun cngstpsstore-new (wanted-changes) ; -> cngstpsstore.
  ...
  (let ((ret-store (make-cngstpsstore :cngstps nil))
	(changes-each-bit (change-split wanted-changes)))
    (loop for one-bit-change in changes-each-bit do
        (push (cngstps-new one-bit-change) (cngstpsstore-cngstps ret-store))
    )
    ret-store
  )
)
...
(defun cngstpsstore-add (storex stpx) ; -> bool, true if added.
  ...
  (let (ret)
    (loop for cngstpsx in (cngstpsstore-cngstps storex) do 
...
(defun cngstpsstore-length (storex) ; -> number.
  ...
  (length (cngstpsstore-cngstps storex))
)
...
(defun cngstpsstore-is-empty (storex) ; -> bool
  ...
  (zerop (length (cngstpsstore-cngstps storex)))
)
...
(defun cngstpsstore-is-not-empty (storex) ; -> bool
  ...
  (plusp (length (cngstpsstore-cngstps storex)))
)
