...
(defun region-tests ()
 ...
 (let (strx) 
   ...
   (assert (and (stringp strx) (string= strx "r01Xx")))
 ...
 (let (errx)
   ...
   (setf errx (region-from-na "r01X3"))
   (assert (and (err-p errx) (string= (err-str errx) "Invalid character 3")))

   ; Test arg with no valid character.
   (setf errx (region-from-na "_"))
   (assert (and (err-p errx) (string= (err-str errx) "No valid character found")))

   ; Test good string.
   (assert (string= (region-str (region-from "r01Xx")) "r01Xx"))
