...
(defun action-structure-needs (actx reachable) ; -> needstore
  ...
  (let ((pairs (regionstore-new nil))               ; All dissimilar square state pairs, so supersets.
    ...
    ;; Separate adjacent from non-adjacent pairs.
    (loop for prx in (regionstore-regions pairs) do
    ...
    (loop for regx in (regionstore-regions critical-non-adj-pairs) do
      (if (not (square-pnc (action-find-square actx (region-first-state regx))))
               (needstore-push needs (action-get-need-resample-state actx (region-first-state regx) *confirm-ip*
      ...
      (if (not (square-pnc (action-find-square actx (region-second-state regx))))
               (needstore-push needs (action-get-need-resample-state actx (region-second-state regx) *confirm-ip*
    ...
    (when (and (not (null (action-logical-structure actx))) (> (regionstore-length (action-logical-structure actx)) 1))
      ...
      (let (defining-regions    ; A regionstore. Regions that have some parts, even as little as a single state, that are only in one region.
        ...
        (let (pairs-in
          ...
          ;; vertexpaths holds data, for the following.
...
(defun action-get-sample (actx stax) ; -> sample
  ...
  (let ((rslt stax) ; If no rule found, default to no change.
        smpl next-val mem-pair)
    (loop for rulsx in (action-base-rules actx)
          for alist in (action-base-memory actx)
          for inx from 0 below (length (action-base-memory actx)) do
        ...
        (when (region-superset-of-state (rule-initial-region (rulestore-nth rulsx 0)) stax) ; All rules in a rulestore should have the same initial region.
           ...
           (cond ((= 1 (rulestore-length rulsx))
                 ...
                 ((= 2 (rulestore-length rulsx))
                   (setf mem-pair (assoc stax alist :test #'state-eq))

                   (cond (mem-pair
                           (setf next-val (mod (+ 1 (cdr mem-pair)) 2))
                           (setf (cdr (assoc stax (nth inx (action-base-memory actx)) :test #'state-eq)) next-val)
                           (setf rslt (rule-result-from-state (rulestore-nth rulsx next-val) stax))
                         )
                         (t (setf next-val (random 2))
                            (push (cons stax next-val) (nth inx (action-base-memory actx)))
                            (setf rslt (rule-result-from-state (rulestore-nth rulsx next-val) stax))
                         )
                   )
                 )
                 ((= 3 (rulestore-length rulsx))
                   ...
                   (cond (mem-pair
                           (setf next-val (mod (+ 1 (cdr mem-pair)) 3))
                           (setf (cdr (assoc stax (nth inx (action-base-memory actx)) :test #'state-eq)) next-val)
                           (setf rslt (rule-result-from-state (rulestore-nth rulsx next-val) stax))
                         )
                         (t (setf next-val (random 3))
                            (push (cons stax next-val) (nth inx (action-base-memory actx)))
                            (setf rslt (rule-result-from-state (rulestore-nth rulsx next-val) stax))
                         )
                   )
