...
for fname in `/usr/bin/grep -v "^;" tmp.txt | /usr/bin/grep "defun " | /usr/bin/cut -d " " -f 2 | /usr/bin/grep -v -e "-print$" | /usr/bin/grep -v -e "^main$" | /usr/bin/grep -v -e "^all-tests$"` 
do
  count=`/usr/bin/grep -c "($fname" tmp.txt`
  if [[ $count -eq 0 ]] || [[ $count2 -eq 0 ]]
  then
