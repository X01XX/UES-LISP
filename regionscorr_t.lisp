...
(defun regionscorr-tests ()
  ...
  (let (regcorr1 regcorr2 regcorr3)
    (setf regcorr1 (regionscorr-new (list (region-from "0X") (region-from "1X"))))
    (setf regcorr2 (regionscorr-new (list (region-from "00") (region-from "X0"))))
    (setf regcorr3 (regionscorr-new (list (region-from "00") (region-from "01"))))
  ...
  (let (regcorr1 regcorr2 regcorr3)
    ...
    (assert (region-eq (regionstore-first-region (regionscorr-regionstore regcorr3)) (region-from "00")))

    (assert (region-eq (regionstore-last-region (regionscorr-regionstore regcorr3)) (region-from "10")))
  ...
  (let (regcorr1 regcorr2 regcorr3)
    (setf regcorr1 (regionscorr-new (list (region-from "0X") (region-from "1X"))))
    (setf regcorr2 (regionscorr-new (list (region-from "0X") (region-from "1X"))))
    (setf regcorr3 (regionscorr-new (list (region-from "00") (region-from "X0"))))
  ...
  (let (regcorr1 regcorr2 list1)
    ...
    (assert (regionscorrstore-contains list1 (regionscorr-new (list (region-from "0X") (region-from "11")))))

    (assert (regionscorrstore-contains list1 (regionscorr-new (list (region-from "01") (region-from "1X")))))
  ...
  (let (regcorr1 msk1 regcorr2)
    (setf regcorr1 (regionscorr-new (list (region-from "01X") (region-from "10X"))))
    (setf msk1 (maskscorr-new (list (mask-from "m111") (mask-from "m010"))))
    (setf regcorr2 (regionscorr-set-to-zeros regcorr1 msk1))
    (assert (regionscorr-eq regcorr2 (regionscorr-new (list (region-from "000") (region-from "10X")))))
  ...
  (let (regcorr1 msk1 regcorr2)
    ...
    (assert (regionscorr-eq regcorr2 (regionscorr-new (list (region-from "111") (region-from "11X")))))
  ...
  (let (regcorr1 regcorr2 regcorr3)
    (setf regcorr1 (regionscorr-new (list (region-from "0_1XX1") (region-from "1_0XX0"))))
    (setf regcorr2 (regionscorr-new (list (region-from "1_01XX") (region-from "1_00XX"))))
    (setf regcorr3 (regionscorr-translate-to regcorr1 regcorr2))
    (assert (regionscorr-eq regcorr3 (regionscorr-new (list (region-from "1_01X1") (region-from "1_00X0")))))
  ...
  (let (regs1 regs2 regs3)
    ...
    (setf regs2 (regionscorr-from "RC[1010]"))
    (assert (= (regionscorr-length regs2) 1))

    (setf regs3 (regionscorr-from "RC[1011, 111]"))
